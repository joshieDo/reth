...
impl WorkerCpuTimer {
    ...
    pub(crate) fn record(
        ...
        tracing::info!(target: "lifecycle", parent: parent, stage, worker_run_ns, worker_thread_cpu_ns,
            ...
            worker_inline_storage_attempts = jobs.filter(|j| j.kind == JobKind::Account).map(|j| j.inline_storage_attempts),
            worker_inline_storage_targets = jobs.filter(|j| j.kind == JobKind::Account).map(|j| j.inline_storage_targets),
