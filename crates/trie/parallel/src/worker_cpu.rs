...
impl WorkerCpuTimer {
    ...
    pub(crate) fn record(
        ...
        tracing::info!(target: "lifecycle", parent: parent, stage, worker_run_ns, worker_thread_cpu_ns,
            ...
            worker_forward_attempts = jobs.filter(|j| j.kind == JobKind::Account).map(|j| j.forward_attempts),
            worker_pressure_fallback_attempts = jobs.filter(|j| j.kind == JobKind::Account).map(|j| j.pressure_fallback_attempts),
